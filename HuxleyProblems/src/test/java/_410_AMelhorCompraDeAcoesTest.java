...
public class _410_AMelhorCompraDeAcoesTest {
	...
	static void setupAll() throws IOException, InterruptedException {
		oracle = new NelsonOracle(basePath + "_410_AMelhorCompraDeAcoes.cpp");
		random.setSeed(seed);
	...
	private void generateInput() throws IOException {
		...
		Integer n = random.nextInt((int) 1e5 - 1) + 1;
		bufferedWriter.write(n.toString() + "\n");
		for (int i = 0; i < n; i ++)
			bufferedWriter.write(Integer.toString(random.nextInt((int) 1e5)) + (i < n - 1 ? " " : "\n"));
